...
class _VoiceContentWidgetState extends State<VoiceContentWidget> {
  ...
  void didUpdateWidget(covariant VoiceContentWidget oldWidget) {
    ...
    if (oldWidget.contentModel.filePath != widget.contentModel.filePath) {
      _isCached = false;
  ...
  Future<void> checkCachingFile() async {
    getCachedFile(
      "${widget.contentId}.${getFileExtension(widget.contentModel.filePath)}",
      FileType.audio,
    ).then((value) {
      if (value != null) {
        _isCached = true;
  ...
  Future<void> downloadMedia() async {
    ...
    _isCached = await MediaHandler().downloadMedia(
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder2<CurrentPlayingAudio?, AudioState>(
      first: voiceController.currentPlayingAudio,
      second: voiceController.audioStateNotifier,
            ...
            child: SfSliderTheme(
              ...
              child: StreamBuilder(
                stream: isSelectedVoice(currentPlayingAudio)
                    ? voiceController.player.positionStream
                    : Stream.value(0),
...
class VoiceButtonStateWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (isLoading) {
      ...
      //   MediaProgressWidget<VoiceHandler>(
      //   contentId: contentId,
